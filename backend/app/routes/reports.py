...
def get_auto_key_reports(
    ...
    all_jobs = list(session.exec(select(AutoKeyJob).where(AutoKeyJob.tenant_id == tenant_id)).all())
    ...
    week_jobs_list: list[int] = []
    week_revenue_list: list[int] = []
    for i in range(7, -1, -1):
        ...
        w_jobs = session.exec(
            ...
            .where(AutoKeyJob.tenant_id == tenant_id)
            .where(AutoKeyJob.created_at >= week_start)
            .where(AutoKeyJob.created_at <= week_end)
        ).one()
        week_jobs_list.append(int(w_jobs))

        inv_rev = session.exec(
            select(func.coalesce(func.sum(AutoKeyInvoice.total_cents), 0))
            .select_from(AutoKeyInvoice)
            .where(AutoKeyInvoice.tenant_id == tenant_id)
            .where(AutoKeyInvoice.created_at >= week_start)
            .where(AutoKeyInvoice.created_at <= week_end)
        ).one()
        week_revenue_list.append(int(inv_rev))
