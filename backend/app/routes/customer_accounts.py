...
def list_customer_accounts(
    ...
    membership_rows = session.exec(
        ...
        .where(CustomerAccountMembership.customer_account_id.in_(account_ids))
        .order_by(CustomerAccountMembership.created_at)
    ).all()
    customer_ids_by_account: dict[UUID, list[UUID]] = {aid: [] for aid in account_ids}
    for row in membership_rows:
        customer_ids_by_account.setdefault(row.customer_account_id, []).append(row.customer_id)

    result: list[CustomerAccountRead] = []
    for account in accounts:
        ...
        result.append(
            CustomerAccountRead(
                ...
                is_active=account.is_active,
                created_at=account.created_at,
                customer_ids=customer_ids_by_account.get(account.id, []),
                account_type=account_type,
                fleet_size=account.fleet_size,
                primary_contact_name=account.primary_contact_name,
                primary_contact_phone=account.primary_contact_phone,
...
def list_customer_account_invoices(
    ...
    line_rows = session.exec(
        select(CustomerAccountInvoiceLine)
        .where(CustomerAccountInvoiceLine.tenant_id == auth.tenant_id)
        .where(CustomerAccountInvoiceLine.customer_account_invoice_id.in_(invoice_ids))
        .order_by(CustomerAccountInvoiceLine.created_at)
    ).all()
    lines_by_invoice: dict[UUID, list[CustomerAccountInvoiceLine]] = {iid: [] for iid in invoice_ids}
    for line in line_rows:
        lines_by_invoice.setdefault(line.customer_account_invoice_id, []).append(line)

    result: list[CustomerAccountInvoiceRead] = []
    for invoice in invoices:
        rows = lines_by_invoice.get(invoice.id, [])
        result.append(
            CustomerAccountInvoiceRead(
                id=invoice.id,
                tenant_id=invoice.tenant_id,
