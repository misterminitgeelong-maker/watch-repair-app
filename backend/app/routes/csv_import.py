...
from ..dependencies import AuthContext, PLAN_FEATURES, enforce_plan_limit, require_manager_or_above
...
async def import_csv(
    ...
    raw_bytes = await file.read(IMPORT_MAX_UPLOAD_BYTES + 1)
    if len(raw_bytes) > IMPORT_MAX_UPLOAD_BYTES:
        raise HTTPException(status_code=413, detail="That file is too large to import (25 MB maximum).")
...
# ── Per-target import strategies ──────────────────────────────────────────────
# _import_csv_sync handled watch, shoe and mobile in one 581-line body: a single
# if/elif over the target inside a 490-line try block. The three branches were
# already independent; they only shared the counters below.


@dataclass
class ImportRunState:
    """Counters and caches shared by a single import run.

    These were locals in _import_csv_sync mutated by whichever branch ran. They
    are bundled rather than passed as eight separate in/out parameters, which
    Python cannot express anyway — the strategies mutate this in place.
    """

    customer_cache: dict[tuple[str, str | None], Customer] = field(default_factory=dict)
    imported: int = 0
    job_seq: int = 0
    duplicate_customer_rows_in_file: int = 0
    job_number_usage: dict[str, int] = field(default_factory=dict)


def _import_watch_rows(
    ...
    *,
    ...
    """Create watch repair jobs from the parsed rows.

    Mutates ``state`` in place — counts, the customer cache and the job-number
    allocations are shared with the caller and with the import log summary.
    """
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if cache_key in state.customer_cache:
            customer = state.customer_cache[cache_key]
            state.duplicate_customer_rows_in_file += 1
        else:
            ...
            session.flush()
            state.customer_cache[cache_key] = customer
        ...
        if original_job_id:
            ...
            if ticket:
                ticket_stem = ticket
        preferred_num = _allocate_import_job_number(state.job_number_usage, ticket_stem, state.job_seq)
...
def _import_shoe_rows(
    ...
    *,
    ...
    """Create shoe repair jobs from the parsed rows.
...
def _import_mobile_rows(
    ...
    *,
    ...
    """Create mobile services (auto key) jobs from the parsed rows.
...
_IMPORT_STRATEGIES = {
    "watch": _import_watch_rows,
    "shoe": _import_shoe_rows,
    "mobile": _import_mobile_rows,
}
...
def _import_csv_sync(
    *,
    ...
    try:
        strategy = _IMPORT_STRATEGIES[import_target]
        strategy(
            state,
            session=session,
            rows=rows,
        ...
        if dry_run:
            ...
            return ImportSummaryResponse(
                ...
                skipped=skipped,
                customers_created=len(state.customer_cache),
                total_rows=len(rows),
                skipped_reasons=skipped_reasons,
                dry_run=True,
                duplicate_customer_rows_in_file=state.duplicate_customer_rows_in_file,
                source_sheet=source_sheet,
                import_target=import_target,
            )

        import_log.imported_count = state.imported
        import_log.skipped_count = skipped
        import_log.customers_created_count = len(state.customer_cache)
