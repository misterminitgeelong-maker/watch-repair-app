...
function CreateCustomerAccountModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} title="Create Customer Account">
      <div className="space-y-3">
        <Input label="Account name *" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} maxLength={128} />
        <div className="grid grid-cols-2 gap-3">
          <Input label="Account code" value={form.account_code} onChange={e => setForm(f => ({ ...f, account_code: e.target.value }))} maxLength={32} help="Optional short code for this account." />
          <Input label="Contact name" value={form.contact_name} onChange={e => setForm(f => ({ ...f, contact_name: e.target.value }))} maxLength={64} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Input label="Contact phone" type="tel" value={form.contact_phone} onChange={e => setForm(f => ({ ...f, contact_phone: e.target.value }))} maxLength={32} />
