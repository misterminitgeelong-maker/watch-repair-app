...
export default function ApprovePage() {
  ...
  if (q.status === 'expired') {
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: 'var(--ms-bg)' }}>
        <div className="max-w-md text-center">
          <Clock className="mx-auto mb-4" size={40} style={{ color: 'var(--ms-text-muted)' }} />
          <h1 className="text-xl font-semibold mb-2" style={{ color: 'var(--ms-text)' }}>This quote link has expired</h1>
          <p style={{ color: 'var(--ms-text-muted)' }}>Please contact the shop for an updated quote or approval link.</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-md text-center">
          {decision === 'approved' ? (
            <>
              ...
              <h1 className="text-2xl font-bold mb-2" style={{ color: 'var(--ms-text)' }}>Quote Approved!</h1>
              <p className="mb-3" style={{ color: 'var(--ms-text-muted)' }}>Thank you — we've received your approval and will begin work shortly.</p>
              <p className="text-sm" style={{ color: 'var(--ms-text-muted)' }}>You'll receive a message when your item is ready for collection. You can also track progress using the status link that was sent with your quote.</p>
            </>
          ) : (
            <>
              <XCircle className="mx-auto mb-4" size={48} style={{ color: 'var(--ms-text-muted)' }} />
              <h1 className="text-2xl font-bold mb-2" style={{ color: 'var(--ms-text)' }}>Quote Declined</h1>
              <p style={{ color: 'var(--ms-text-muted)' }}>We've noted your decision. Please contact us if you'd like to discuss alternatives.</p>
            </>
          )}
        </div>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: 'var(--ms-bg)' }}>
        <div className="max-w-md text-center">
          {q.status === 'approved'
            ? <CheckCircle className="mx-auto mb-4 text-green-500" size={48} />
            : <XCircle className="mx-auto mb-4" size={48} style={{ color: 'var(--ms-text-muted)' }} />}
          <h1 className="text-xl font-semibold mb-2 capitalize" style={{ color: 'var(--ms-text)' }}>Quote already {q.status}</h1>
          <p style={{ color: 'var(--ms-text-muted)' }}>This quote has already been responded to. Please contact the shop if you have questions.</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="rounded-2xl shadow-sm overflow-hidden mb-5" style={{ backgroundColor: 'var(--ms-surface)', border: '1px solid var(--ms-border)' }}>
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: 'var(--ms-bg)', borderBottom: '1px solid var(--ms-border)' }}>
                <th className="px-5 py-3 text-left text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--ms-text-muted)' }}>Item</th>
                <th className="px-5 py-3 text-right text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--ms-text-muted)' }}>Qty</th>
                <th className="px-5 py-3 text-right text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--ms-text-muted)' }}>Unit</th>
                <th className="px-5 py-3 text-right text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--ms-text-muted)' }}>Total</th>
              </tr>
            </thead>
            <tbody>
              {q.line_items.map((li, i) => (
                <tr key={i} style={{ borderBottom: '1px solid var(--ms-border)' }}>
                  <td className="px-5 py-3">
                    <p className="font-medium" style={{ color: 'var(--ms-text)' }}>{li.description}</p>
                    <p className="text-xs capitalize" style={{ color: 'var(--ms-text-muted)' }}>{li.item_type}</p>
                  </td>
                  <td className="px-5 py-3 text-right" style={{ color: 'var(--ms-text-mid)' }}>{li.quantity}</td>
                  <td className="px-5 py-3 text-right" style={{ color: 'var(--ms-text-mid)' }}>{formatCents(li.unit_price_cents)}</td>
                  <td className="px-5 py-3 text-right font-medium" style={{ color: 'var(--ms-text)' }}>{formatCents(li.total_price_cents)}</td>
                </tr>
              ))}
            </tbody>
          </table>

          <div className="px-5 py-4 space-y-1.5" style={{ borderTop: '1px solid var(--ms-border)', backgroundColor: 'var(--ms-bg)' }}>
            ...
            <div className="flex justify-between font-bold text-base pt-1" style={{ borderTop: '1px solid var(--ms-border)', color: 'var(--ms-text)' }}>
        ...
        <div className="mb-5">
          <p className="text-sm font-medium mb-2" style={{ color: 'var(--ms-text-muted)' }}>Sign to approve (optional)</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            style={{ border: '2px solid var(--ms-border-strong)', color: 'var(--ms-text-mid)' }}
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'var(--ms-bg)'; e.currentTarget.style.borderColor = 'var(--ms-accent)' }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.borderColor = 'var(--ms-border-strong)' }}
          >
