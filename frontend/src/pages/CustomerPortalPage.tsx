    ...
    <div className="space-y-3">
      ...
      {jobs.map((job, idx) => (
        <Link
          ...
          style={{ backgroundColor: 'var(--cafe-surface)', border: '1px solid var(--cafe-border)', textDecoration: 'none' }}
        >
          <div className="shrink-0 w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: '#EEE6DA' }}>
            {job.type === 'shoe'
              ? <Footprints size={18} style={{ color: 'var(--cafe-gold-dark)' }} />
              : <Watch size={18} style={{ color: 'var(--cafe-gold-dark)' }} />
            }
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold truncate" style={{ color: 'var(--cafe-text)' }}>{job.title}</p>
            ...
            <p className="text-xs mt-0.5 capitalize font-medium" style={{ color: 'var(--cafe-amber)' }}>
              {readableStatus(job.status)}
            </p>
          </div>
          <ArrowRight size={16} style={{ color: 'var(--cafe-text-muted)', flexShrink: 0 }} />
        </Link>
      ))}
    </div>
  )
  ...
  return (
    <div className="rounded-xl p-4" style={{ backgroundColor: 'var(--cafe-surface)', border: '1px solid var(--cafe-border)' }}>
      <div className="flex items-start gap-3">
        <Link2 size={18} style={{ color: 'var(--cafe-amber)', flexShrink: 0, marginTop: 2 }} />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold" style={{ color: 'var(--cafe-text)' }}>Bookmark this page</p>
          <p className="text-xs mt-0.5" style={{ color: 'var(--cafe-text-muted)' }}>
            Save this link to check your repairs any time — valid for 30 days.
          </p>
          <div className="flex items-center gap-2 mt-2">
            <span className="text-xs truncate flex-1 font-mono" style={{ color: 'var(--cafe-text-mid)' }}>{url}</span>
            <button
              type="button"
              onClick={copy}
              className="flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-lg shrink-0"
              style={{ backgroundColor: copied ? 'rgba(31,109,76,0.12)' : 'var(--cafe-bg)', color: copied ? '#1F6D4C' : 'var(--cafe-amber)', border: '1px solid var(--cafe-border-2)' }}
            >
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: 'var(--cafe-bg)' }}>
        <div className="max-w-md text-center space-y-3">
          <p className="text-lg font-semibold" style={{ color: 'var(--cafe-text)' }}>Link expired</p>
          <p className="text-sm" style={{ color: 'var(--cafe-text-muted)' }}>{error}</p>
          <Link to="/customer-portal" className="text-sm font-medium" style={{ color: 'var(--cafe-amber)' }}>
  ...
  if (jobs === null) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--cafe-bg)' }}>
        <p className="text-sm" style={{ color: 'var(--cafe-text-muted)' }}>Loading your repairs…</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen py-10 px-4" style={{ backgroundColor: 'var(--cafe-bg)' }}>
      <div className="max-w-lg mx-auto space-y-5">
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-1" style={{ fontFamily: "'Playfair Display', Georgia, serif", color: 'var(--cafe-text)' }}>My Repairs</h1>
          <p className="text-sm" style={{ color: 'var(--cafe-text-muted)' }}>{email}</p>
        </div>
        {jobs.length === 0 ? (
          <p className="text-center text-sm" style={{ color: 'var(--cafe-text-muted)' }}>No active repairs found.</p>
        ) : (
          <JobList jobs={jobs} />
        )}
      </div>
  ...
  return (
    <div className="min-h-screen py-10 px-4" style={{ backgroundColor: 'var(--cafe-bg)' }}>
      <div className="max-w-lg mx-auto space-y-6">
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif", color: 'var(--cafe-text)' }}>
            Track Your Repairs
          </h1>
          <p style={{ color: 'var(--cafe-text-muted)' }}>Enter your email to see all your active repairs.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input
            ...
            style={{ backgroundColor: 'var(--cafe-surface)', border: '1px solid var(--cafe-border-2)', color: 'var(--cafe-text)', outline: 'none' }}
          />
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-opacity"
            style={{ backgroundColor: 'var(--cafe-amber)', color: '#fff', opacity: loading ? 0.7 : 1 }}
          >
            <Search size={15} />
        ...
        {jobs !== null && (
          <div className="space-y-4">
            {jobs.length === 0 ? (
              <p className="text-center text-sm" style={{ color: 'var(--cafe-text-muted)' }}>
