...
export default function ShoeApprovePage() {
  ...
  const bg = { backgroundColor: 'var(--ms-bg)' }
  const surface = { backgroundColor: 'var(--ms-surface)', border: '1px solid var(--ms-border)' }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={bg}>
        <div className="max-w-md text-center">
          ...
          <h1 className="text-xl font-semibold mb-2 capitalize" style={{ color: 'var(--ms-text)' }}>
            Quote already {q.quote_status}
          </h1>
          <p style={{ color: 'var(--ms-text-muted)' }}>This quote has already been responded to. Contact the shop if you have questions.</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-md text-center">
          {decision === 'approved' ? (
            <>
              ...
              <p style={{ color: 'var(--ms-text-muted)' }}>Thank you — we've received your approval and will get started on your shoes shortly.</p>
            </>
          ) : (
            <>
              <XCircle className="mx-auto mb-4" size={48} style={{ color: 'var(--ms-text-muted)' }} />
    ...
    <div className="min-h-screen py-10 px-4" style={bg}>
      <div className="max-w-xl mx-auto space-y-5">
        ...
        <div className="rounded-xl p-5 space-y-3" style={surface}>
          <h2 className="text-sm font-semibold uppercase tracking-widest" style={{ color: 'var(--ms-text-muted)' }}>Services</h2>
          {q.items.length === 0 ? (
            <p className="text-sm" style={{ color: 'var(--ms-text-mid)' }}>Quote details are being finalised.</p>
          ) : (
            q.items.map((item, idx) => (
              <div key={`${item.item_name}-${idx}`} className="flex justify-between items-start text-sm gap-4">
                <div>
                  ...
                  {item.notes && <p style={{ color: 'var(--ms-text-muted)' }}>{item.notes}</p>}
                </div>
                <span className="whitespace-nowrap" style={{ color: 'var(--ms-text-muted)' }}>
                  {item.unit_price_cents == null ? 'TBC' : formatCents(item.unit_price_cents * item.quantity)}
                </span>
              </div>
            ))
          )}
          <div className="pt-3 border-t flex justify-between text-sm font-semibold" style={{ borderColor: 'var(--ms-border)' }}>
            <span style={{ color: 'var(--ms-text)' }}>Total (est.)</span>
            <span style={{ color: 'var(--ms-accent)' }}>{formatCents(q.subtotal_cents)}</span>
          </div>
        </div>

        {q.description && (
          <div className="rounded-xl p-5" style={surface}>
            <h2 className="text-sm font-semibold uppercase tracking-widest mb-2" style={{ color: 'var(--ms-text-muted)' }}>Notes</h2>
            <p className="text-sm" style={{ color: 'var(--ms-text-mid)' }}>{q.description}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            ...
            style={{ border: '1px solid var(--ms-border-strong)', color: 'var(--ms-text-muted)', backgroundColor: 'var(--ms-surface)', opacity: mut.isPending ? 0.6 : 1 }}
          >
            Decline
          </button>
          <button
            ...
            disabled={mut.isPending}
            className="py-3 rounded-xl text-sm font-semibold transition-opacity"
            style={{ backgroundColor: 'var(--ms-accent)', color: '#fff', opacity: mut.isPending ? 0.6 : 1 }}
          >
