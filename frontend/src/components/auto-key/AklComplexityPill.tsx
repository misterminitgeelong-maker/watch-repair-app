...
function styleForComplexity(complexity: string): { bg: string; color: string } {
  ...
  if (c.includes('very high') || c.includes('refer')) return { bg: 'color-mix(in srgb, var(--ms-error) 15%, transparent)', color: 'var(--ms-error)' }
  if (c.includes('high')) return { bg: 'color-mix(in srgb, var(--ms-error) 10%, transparent)', color: '#B85A4A' }
