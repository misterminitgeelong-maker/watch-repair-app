...
import {
  ...
  type VswtKpiGroup, type VswtUnavailable, type VswtWeeklyReport, type VswtWeeklyReportShop,
} from '@/lib/api'
 ...
 * franchisee group, say), and get one week's numbers for just those shops laid out together —
 * a PDF you can drop straight into a group chat. Picks and title are remembered per-tenant so
 * this doesn't need rebuilding from scratch every week. */
export function VswtWeeklyReportBuilder() {
  ...
  const { data: reportData, isLoading: reportLoading, isFetching: reportFetching } = useQuery({
    queryKey: ['vswt-weekly-report', shopNumbers, week, group],
    queryFn: () => getVswtWeeklyReport({ shopNumbers, week, group }).then(r => r.data),
    enabled: shopNumbers.length > 0,
  })

  const downloadMut = useMutation({
    mutationFn: () => getVswtWeeklyReportPdf({ shopNumbers, week: reportData?.available ? reportData.week : week, group, title }),
    onSuccess: r => {
  ...
  return (
    <div>
      <p className="text-sm mb-4" style={{ color: 'var(--ms-text-muted)' }}>
        ...
        ready to drop into your group chat. Your picks and title are remembered for next time.
      </p>
...
function WeeklyReportPreview({
  reportData: data, isLoading, isFetching, title, onWeekChange, group, onGroupChange, onDownload, downloading,
}: {
  ...
  group: VswtKpiGroup
  onGroupChange: (g: VswtKpiGroup) => void
  onDownload: () => void
  downloading: boolean
}) {
  ...
  const extraKpis = data.kpis.filter(k => k.key !== 'sales_ty')
    ...
    <Card className="p-5">
      <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <PillToggle value={group} onChange={onGroupChange} options={VSWT_KPI_GROUPS.map(g => ({ key: g, label: g }))} />
        ...
        <table className="border-collapse text-sm" style={{ width: '100%' }}>
          <thead>
            <tr style={{ background: 'var(--ms-bg)' }}>
              ...
              {extraKpis.map(k => <th key={k.key} style={thStyle}>{k.label}</th>)}
            </tr>
          </thead>
          <tbody>
            {data.shops.map(s => <ShopRow key={s.shop_number} shop={s} extraKpis={extraKpis} />)}
          </tbody>
          <tfoot>
            <tr>
              ...
              {extraKpis.map(k => (
                <td key={k.key} style={{ ...tdStyle, fontWeight: 700, color: 'var(--ms-text)' }}>
                  {k.key === 'customer_ty' ? fmtVswtVal(data.totals.customers, 'count')
                    : k.key === 'jobs_ty' ? fmtVswtVal(data.totals.jobs, 'count')
                    : ''}
                </td>
              ))}
            </tr>
...
function ShopRow({ shop, extraKpis }: { shop: VswtWeeklyReportShop; extraKpis: { key: string; type: 'currency' | 'percent' | 'count' | 'ratio' }[] }) {
  return (
    <tr style={shop.is_me ? { backgroundColor: 'var(--ms-accent-light)' } : undefined}>
      <td style={{ ...tdStyle, textAlign: 'left', color: shop.is_me ? 'var(--ms-accent)' : 'var(--ms-text)', fontWeight: 600 }}>
        <span className="flex items-center gap-1.5">
          {shop.shop_name ?? shop.shop_number}
          {shop.is_me && <Badge variant="default">You</Badge>}
        </span>
      </td>
      <td style={{ ...tdStyle, textAlign: 'left', color: 'var(--ms-text-mid)' }}>{shop.area_name ?? '—'}</td>
      <td style={{ ...tdStyle, color: 'var(--ms-text)', fontWeight: 600 }}>{fmtVswtVal(shop.sales_value, 'currency')}</td>
      <td style={tdStyle}>{shop.sales_rank != null ? <Badge variant="default">#{shop.sales_rank}</Badge> : <span style={{ color: 'var(--ms-text-muted)' }}>—</span>}</td>
      {extraKpis.map(k => (
        <td key={k.key} style={{ ...tdStyle, color: 'var(--ms-text)' }}>{fmtVswtVal(shop.values[k.key], k.type)}</td>
      ))}
    </tr>
  )
}
