...
import {
  ...
  customerName,
  isMobileVisitJob as _isMobileVisitJob,
  isValidPermutation,
  loadGeocodeCache,
  MELBOURNE_CENTRE,
  saveGeocodeCache,
  sortJobsBySchedule,
  vehicleLabel,
  type MapCustomer as Customer,
  type MapJob as Job,
  type MapJobWithAddr as JobWithAddr,
} from '@/components/mobileServicesMap/mapUtils'
...
// Pure helpers live in mobileServicesMap/mapUtils.ts — see top-of-file import.
// Local adapter keeps the existing inline call sites terse (they used
// `isMobileVisitJob(j)` without the mobile-job-types set):
function isMobileVisitJob(j: Job): boolean {
  return _isMobileVisitJob(j, MOBILE_JOB_TYPES)
}
